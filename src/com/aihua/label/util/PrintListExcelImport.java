...
public class PrintListExcelImport {
	...
	private static LabelDao labelPrintLogDao = new LabelDao();
	private static AiHuaLabel labelPrintLog = new AiHuaLabel();
	...
	public PrintListExcelImport() {
		...
		if (returnValue == fileChooser.APPROVE_OPTION) {
			...
			for (int i = 1; i < rowSize; i++)// 从第1行开始，过滤表头
			{
				...
				labelPrintLog.setNetWeight(sheet.getCell(5, i).getContents());
				labelPrintLog.setGrossWeight(sheet.getCell(6, i).getContents());
				...
				labelPrintLog.setProductionDate(sheet.getCell(14, i).getContents());
