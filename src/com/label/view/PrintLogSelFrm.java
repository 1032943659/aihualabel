...
import com.label.util.DateChooser;
...
public class PrintLogSelFrm extends JFrame {
	...
	public static LabelPrintLog outlabelPrintLogSelect = new LabelPrintLog();//用于存放选定的外箱打印记录
	...
	public PrintLogSelFrm() {
		...
		createOutBox_Button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				ArrayList<LabelPrintLog> insideBox_labelPrintLog = new ArrayList<LabelPrintLog>();// 外箱获取内箱打印数据
				if(BoxType_comboBox.getSelectedItem()=="内箱"){
					...
					for (int i = 0; i < Row.length; i++) {
						...
						labelPrintLog.setN_Id(Integer.valueOf(PrintLog_table.getValueAt(Row[i], 0)+""));
						...
						labelPrintLog.setNet_Weight((String) PrintLog_table.getValueAt(Row[i], 12));
						labelPrintLog.setGross_Weight((String) PrintLog_table.getValueAt(Row[i], 13));
						labelPrintLog.setProduction_Date((String) PrintLog_table.getValueAt(Row[i], 14));
...
//						JOptionPane.showMessageDialog(null, "");
		...
		TagPrinted_Button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (PrintLogSelFrm.BoxType_comboBox.getSelectedItem() == "内箱"&&rows.length!=0) {
					...
					if (result == 0) {
						ArrayList<LabelPrintLog> outPrinted = new ArrayList<LabelPrintLog>();
		...
		PrintLog_table.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent e) {
				...
				if (row != -1&&BoxType_comboBox.getSelectedItem()=="内箱"){
					...
					outlabelPrintLogSelect.setNet_Weight((String) PrintLog_table.getValueAt(row, 12));
					outlabelPrintLogSelect.setGross_Weight((String) PrintLog_table.getValueAt(row, 13));
					outlabelPrintLogSelect.setProduction_Date((String) PrintLog_table.getValueAt(row, 14));
	...
	protected void LabelPringLogSearchActionPerformedByPD(DocumentEvent e) {
		...
		LabelPrintLog labelPrintLog = new LabelPrintLog();
		labelPrintLog.setProduction_Date(this.PD_textField.getText());
		if(PrintLogSelFrm.BoxType_comboBox.getSelectedIndex()==0){
			labelPrintLog.setBoxType(0);
		}else if(PrintLogSelFrm.BoxType_comboBox.getSelectedIndex()==1){
			labelPrintLog.setBoxType(1);
	 ...
	 * @throws SQLException 
	 */
	public static void fillTable(LabelPrintLog labelPrintLog) throws SQLException{
		LabelPrintLogDao labelPrintLogDao = new LabelPrintLogDao();
